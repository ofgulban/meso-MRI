...
OUTFILE = "/home/faruk/data2/ISMRM-2022/anim-slice_slide/anim_prep/scene-invivo_cake_shot-3.nii.gz"
...
norm = np.linalg.norm(indices, axis=2)
norm[norm == 0] = np.min(norm[norm > 0])
norm[~mask] = 0
...
norm *= -NR_CAKE_LAYERS
norm = np.floor(norm)
norm[mask] = norm[mask] + NR_CAKE_LAYERS
norm = np.repeat(norm[..., None], dims[2], axis=2)
