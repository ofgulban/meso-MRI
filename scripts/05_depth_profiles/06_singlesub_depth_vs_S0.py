"""Single subject layer profile plots. Added based on reviewer's request."""
...
OUTDIR = "/home/faruk/data2/DATA_MRI_NIFTI/derived/plots/22_depth_vs_S0"
FIGURE_TAG = "depth_vs_S0"
...
for s in SUBJ_ID:
    ...
    METRIC_Y = "/home/faruk/data2/DATA_MRI_NIFTI/derived/{}/T2s/12_T2star/{}_ses-T2s_part-mag_MEGRE_crop_ups2X_prepped_avg_composite_decayfixed_S0.nii.gz".format(s, s)
    ...
    VMIN, VMAX = 0, 200  # Histogram bin count min and max
    CMAP = cc.cm.fire
    # =========================================================================
    ...
    for i in range(len(CHUNKS)):
        ...
        idx2 = np.zeros((np.sum(idx), 3), dtype=bool)
    ...
    for i in range(len(CHUNKS)):
        ...
        # Normalize median to histogram bin indices
        median_line_x = ((median_x - RANGE_Y[0])
                         / (RANGE_Y[1] - RANGE_Y[0])) * nr_bins
        ax[i].plot((0, 50), (median_line_x, median_line_x),
                   '-', linewidth=1.5, color="white")
        ax[i].text(0+1, median_line_x + 1, '{:.0f}'.format(median_x),
                   fontsize=10, color="white")
        ...
        ax[i].plot((50, 150), (median_line_x, median_line_x),
                   '-', linewidth=1.5, color="white")
        ax[i].text(50+1, median_line_x + 1, '{:.0f}'.format(median_x),
                   fontsize=10, color="white")
        ...
        ax[i].plot((150, 200), (median_line_x, median_line_x),
                   '-', linewidth=1.5, color="white")
        ax[i].text(150+1, median_line_x + 1, '{:.0f}'.format(median_x),
                   fontsize=10, color="white")
